...
<template>
  ...
  >
    <!-- Top Image Container -->
    <div class="relative w-full aspect-square rounded-xl overflow-hidden bg-slate-100/70 mb-3.5">
      <img
        :src="toy.imageUrl"
        :alt="toy.name"
      ...
      >
        <svg
          ...
          :class="wishlistStore.isFavorite(toy.id) ? 'text-rose-600 fill-rose-600' : ''"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    <div class="space-y-2 flex-1 flex flex-col justify-between">
      <div>
        <div class="flex items-center justify-between gap-1 text-[11px] font-semibold text-slate-400">
          <span
            v-if="tcgInfo"

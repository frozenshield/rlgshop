...
<template>
  <teleport to="body">
    ...
    >
      ...
      >
        <div
          class="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative border-2 border-red-100 transform transition-all my-8 max-h-[90vh] overflow-y-auto"
          @click.stop
        >
          <!-- Close Button -->
          <button
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div class="space-y-3">
              <div class="aspect-square rounded-2xl overflow-hidden bg-amber-50/60 border border-slate-100 relative">
                <img
                  :src="currentImage"
                  :alt="toy.name"
              ...
              <div v-if="toy.galleryImages && toy.galleryImages.length > 1" class="flex gap-2">
                <img
                  ...
                  :class="currentImage === img ? 'border-red-500 scale-105' : 'border-slate-200 opacity-70 hover:opacity-100'"
            ...
            <div class="space-y-4">
              <div>
                <div class="flex items-center gap-2 mb-1 flex-wrap">
                  <span
                    v-if="tcgInfo"
                    class="inline-flex items-center gap-1 font-extrabold text-xs px-2.5 py-0.5 rounded-full border shadow-2xs"
                    :class="tcgInfo.bgClass"
                  >
                    <span>{{ tcgInfo.icon }}</span>
                    <span>{{ tcgInfo.name }}</span>
                  </span>
                  <span v-else class="text-xs font-extrabold text-red-600 uppercase tracking-wider">
              ...
              <!-- Price Box -->
              <div class="flex items-baseline gap-3 p-3 rounded-2xl bg-amber-50/70 border border-amber-200">
                ...
                <span v-if="toy.stock > 0" class="text-xs font-bold text-emerald-600 ml-auto">
                  ✓ Available to Catch ({{ toy.stock }} units)
                </span>
              </div>
              ...
              <div
                ...
                class="p-2.5 rounded-xl bg-orange-50 border border-orange-200 text-[11px] text-orange-800 font-semibold flex items-center gap-2"
              >
                <span>⚠️</span>
                <span>{{ toy.safetyWarning }}</span>
              </div>

              <!-- Quantity Selector & Catch Action -->
              <div class="pt-2 flex items-center gap-3">
                <div class="flex items-center border border-slate-200 rounded-2xl p-1 bg-slate-50">
                  <button
                    type="button"
                    class="w-8 h-8 rounded-xl bg-white shadow-xs font-bold text-slate-700 hover:bg-red-50 hover:text-red-600 flex items-center justify-center cursor-pointer transition-colors"
                    @click="decrementQty"
                  >
                    -
                  </button>
                ...
                <button
                  type="button"
                  class="w-11 h-11 rounded-2xl border border-slate-200 flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-colors cursor-pointer"
                  :title="wishlistStore.isFavorite(toy.id) ? 'Remove from Wishlist' : 'Add to Wishlist'"
                  @click="handleToggleWishlist"
                >
                  <svg
                    class="w-5 h-5 transition-colors"
                    :class="wishlistStore.isFavorite(toy.id) ? 'text-red-600 fill-red-600' : 'text-slate-400'"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >

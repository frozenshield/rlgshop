...
<template>
  <div class="min-h-screen bg-slate-100/70 text-slate-800 flex font-display select-none">
    <!-- Desktop Sidebar -->
    <aside
    ...
    >
      ...
      <div class="h-20 flex items-center justify-between px-5 border-b border-slate-800/80">
        <router-link to="/admin/dashboard" class="flex items-center gap-3 overflow-hidden">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-rose-600 via-slate-800 to-indigo-600 flex items-center justify-center text-white font-black text-lg flex-shrink-0 shadow-md">
        ...
        >
          <svg class="w-4 h-4 transition-transform" :class="isSidebarCollapsed ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        <router-link
          ...
          :class="route.path.startsWith(item.path) ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'"
          :title="item.name"
        >
          <span class="text-lg flex-shrink-0">{{ item.icon }}</span>
          <span v-if="!isSidebarCollapsed" class="flex-1 truncate">{{ item.name }}</span>
          <span
            v-if="!isSidebarCollapsed && item.badge"
            class="text-[10px] font-extrabold px-1.5 py-0.2 rounded-full"
            :class="route.path.startsWith(item.path) ? 'bg-white/20 text-white' : 'bg-slate-800 text-rose-400'"
          >
            {{ item.badge }}
          </span>
        </router-link>
      ...
      <div class="p-3 border-t border-slate-800/80">
        ...
        >
          ...
          <div v-if="!isSidebarCollapsed" class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-slate-400 capitalize truncate">{{ adminStore.currentAdmin?.role || 'Super Admin' }}</p>
          </div>
          <button
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
      <!-- Top Navbar -->
      <header class="h-20 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between gap-4 sticky top-0 z-20 shadow-xs">
    ...
    >
      <div class="w-64 bg-slate-950 text-slate-300 h-full p-4 space-y-4" @click.stop>
        <div class="flex items-center justify-between pb-3 border-b border-slate-800">
          <span class="text-sm font-black text-white">RLG HOBBY OS</span>
          <button type="button" class="text-slate-400" @click="isMobileMenuOpen = false">✕</button>
        </div>
        ...
        <nav class="space-y-1">
          <router-link
            ...
            :class="route.path.startsWith(item.path) ? 'bg-rose-600 text-white' : 'text-slate-400 hover:bg-slate-900'"
            @click="isMobileMenuOpen = false"
          >
            <span>{{ item.icon }}</span>
            <span>{{ item.name }}</span>
